...
def loss_calculation(pred_r, pred_t, pred_c, target, model_points, idx, points, w, refine, num_point_mesh, sym_list,
    ...
    ### Edge Loss
    edge = edge.view(-1)
    pred_edge = pred_edge.view(-1)
    edge_weight = edge_weight.view(-1)
    edge_loss = F.binary_cross_entropy_with_logits(pred_edge, edge, reduction='none')
    edge_loss = edge_loss * edge_weight
    edge_loss = torch.mean(edge_loss)
    ###
    loss = loss + weight_blance * edge_loss
...
class Loss(_Loss):
    ...
    def forward(self, pred_r, pred_t, pred_c, target, model_points, idx, points, w, refine, edge, pred_edge,
                edge_weight,
                weight_blance):
        return loss_calculation(pred_r, pred_t, pred_c, target, model_points, idx, points, w, refine, self.num_pt_mesh,
                                self.sym_list, edge, pred_edge, edge_weight, weight_blance)
